...
def parse_arguments():
    ...
    parser.add_argument('--att', type=str, default='additive', help='Type of attention. Choose from {additive, cosine, dotprod, sdotprod}')
...
def main(args):
    ...
    train_transform, val_transform, train_transform_mask, val_transform_mask = get_transforms_binary_segmentation_attention(args)
    ...
    for epoch in range(epochs):
        ...
        t_loss, t_iou = train(train_loader, model, criterion, optimizer, logger)
        val_loss, val_iou = validate(val_loader, model, criterion, logger, epoch)
        print(f"Training loss: {t_loss} iou:{t_iou}")
